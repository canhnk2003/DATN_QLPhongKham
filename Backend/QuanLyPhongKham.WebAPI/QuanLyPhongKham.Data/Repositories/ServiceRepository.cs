...
namespace QuanLyPhongKham.Data.Repositories
{
    public class ServiceRepository : BaseRepository<DichVu>, IServiceRepository
    {
        ...
        public async Task<string> GetNextMaDichVu(Guid khoaId)
        {
            var khoa = await _context.Khoas.FirstOrDefaultAsync(k => k.KhoaId == khoaId);
            if (khoa == null)
            {
                return "KXXX_DVXXX";
            }
            var maKhoaByDv = khoa.MaKhoa;
            ...
            var maxMaDV = _context.DichVus
                ...
                {
                    MaBN = dv.MaDichVu,
                    So = int.Parse(dv.MaDichVu.Substring(7)) // Tách phần số, bỏ qua 2 ký tự đầu tiên "BN"
                })
                .OrderByDescending(dv => dv.So) // Sắp xếp theo số, giảm dần
                .FirstOrDefault();

            // Nếu không có bệnh nhân nào trong hệ thống, mã bắt đầu từ "KXXXDV001"
            if (maxMaDV == null)
            {
                return maKhoaByDv + "_DV001";
            }
            ...
            return maKhoaByDv + $"_DV{nextSo:D3}";
        }
    }
}
