...
$(document).ready(function () {
  ...
  getAllDoctor(doctorSelect, appointmentTimeSelect, serviceSelect);
  ...
  getAllDoctor(doctorSelectEdit, appointmentTimeSelectEdit, serviceSelectEdit);
  ...
  //Xử lý khi nhấn nút Lấy thông tin lịch khám cũ
  $("#btnGetOldInfor").click(async function () {
    try {
      // Chờ hoàn tất việc lấy thông tin bệnh nhân
      await getPatientByUserId();
      ...
      console.error("Lỗi khi xử lý Lấy thông tin lịch khám gần nhất:", error);
    }
  });
...
function fillEditModal() {
  ...
  if (
    ...
    lichKham.trangThaiLichKham === "Hoàn thành"||lichKham.trangThaiLichKham === "Đã hoàn thành"
  ) {
  ...
  if (bn) {
    ...
    $("#modalEditAppointment #gender-edit").val(benhNhan.loaiGioiTinh ?? "");
    ...
    $("#modalEditAppointment #message-edit").val(benhNhan.tienSuBenhLy || "");
  } else {
...
// Hàm điền thông tin lịch khám cũ điền vào form
function fillInforToForm() {
  if (!lichKham) {
    console.error("Không có thông tin lịch khám để điền!");
    return;
  }
  ...
  const formattedDate = lichKham.ngayKham
    ? new Date(lichKham.ngayKham).toLocaleDateString("en-CA") // Định dạng YYYY-MM-DD theo múi giờ cục bộ
    : "";
  $("#appointment #appointment-date").val(formattedDate);
  ...
  if (doctor && doctor.gioLamViec) {
    ...
    $("#appointment #service").val(lichKham.dichVuId);
  }
  // Xử lý thông tin bệnh nhân
  if (bn) {
    const benhNhan = bn;
    $("#appointment #name").val(benhNhan.hoTen || "");
    $("#appointment #phone").val(benhNhan.soDienThoai || "");
    $("#appointment #email").val(benhNhan.email || "");
    $("#appointment #gender").val(benhNhan.loaiGioiTinh ?? "");
    $("#appointment #dateOfBirth").val(
      benhNhan.ngaySinh
        ? new Date(benhNhan.ngaySinh).toLocaleDateString("en-CA") // Định dạng YYYY-MM-DD theo múi giờ cục bộ
        : ""
    );
    $("#appointment #address").val(benhNhan.diaChi || "");
    $("#appointment #message").val(benhNhan.tienSuBenhLy || "");
  } else {
    // Đặt mặc định nếu không có bệnh nhân
    $("#appointment #name").val("");
    $("#appointment #phone").val("");
    $("#appointment #email").val("");
    $("#appointment #gender").val("");
    $("#appointment #dateOfBirth").val("");
    $("#appointment #address").val("");
...
function checkData(appointment, formPrefix, str) {
  ...
  // Kiểm tra chọn khoa để lọc ra bác sĩ và dịch vụ
  // const khoaValue = $(`#${formPrefix} #department${str}`).val();
  // console.log(khoaValue);
  // if (!khoaValue.trim()) {
  //   setError(
  //     `#${formPrefix} #department${str}`,
  //     "Chọn khoa để chọn bác sĩ và dịch vụ khám tương ứng."
  //   );
  // } else {
  //   clearError(`#${formPrefix} #department${str}`);
  // }
  ...
  // Kiểm tra dịch vụ
  if (!appointment.dichVuId.trim()) {
    setError(`#${formPrefix} #service${str}`, "Dịch vụ không được để trống.");
  } else {
    clearError(`#${formPrefix} #service${str}`);
  }
...
function edit(appointment) {
  ...
  axiosJWT
    ...
    .catch(function (error) {
      ...
      $("#modalEditAppointment .loading").hide();
      const errorDiv =  $("#modalEditAppointment .error-message");
      errorDiv.text("Sửa thông lịch khám thất bại!")
      errorDiv.show();
      setTimeout(function () {
        errorDiv.hide();
      }, 5000); // 5000ms = 5 giây
...
function register(appointment) {
  ...
  axiosJWT
    ...
    .catch(function (error) {
      ...
      $("#appointment .loading").hide();
      const errorDiv = $("#appointment .error-message");
      errorDiv.text("Đăng ký lịch khám thất bại!")
      errorDiv.show();
      setTimeout(function () {
        // Xóa nội dung input
        $("#appointment input").val("");
...
// Lấy toàn bộ Bác sĩ
function getAllDoctor(selectElement, appointmentTimeSelect, serviceSelect) {
  axiosJWT
    .get(`/api/Doctors`)
    .then(function (response) {
      ...
      selectElement.on("change", () => {
        addEventSelect(selectElement, dsBacSi, appointmentTimeSelect, serviceSelect);
      });
      ...
      axiosJWT
        ...
        .then(function (response) {
          ...
          doctorSelect.off("change").on("change", function () {
            addEventSelect(doctorSelect, dsBacSi, appointmentTimeSelect, serviceSelect);
          });
        })
        .catch(function (error) {
...
function addEventSelect(selectElement, dsBacSi, appointmentTimeSelect, serviceSelect) {
  ...
  if (selectedDoctorId) {
    ...
    if (selectedDoctor) {
      ...
      // ✅ Đổ dịch vụ theo khoaId của bác sĩ
      if (selectedDoctor.khoaId) {
        axiosJWT
          .get(`/api/Services/${selectedDoctor.khoaId}`)
          .then(function (response) {
            const dsDichVu = response.data;
            dsDichVu.forEach((item) => {
              const option = $("<option>")
          ...
          .catch(function (error) {
            console.error("Lỗi khi lấy dịch vụ theo bác sĩ:", error);
      ...
      appointmentTimeSelect.attr("disabled", true);
      serviceSelect.attr("disabled", true);
    }
  } else {
    // Không chọn bác sĩ
